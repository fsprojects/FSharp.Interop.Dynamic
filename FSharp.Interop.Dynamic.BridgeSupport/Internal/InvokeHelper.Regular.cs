...
// commit 38fcb82, Internal/Optimization/InvokeHelper-Regular.cs). Trimmed to
// what FSharp.Interop.Dynamic calls; see THIRD-PARTY-NOTICES.txt.
// Dropped: BuildProxy / EmitCallSiteFuncType (ImpromptuInterface late-bind),
// IsActionOrFunc, InvokeMethodDelegate, DynamicInvokeStaticMember and the
// constructor call site, DynamicAction/DynamicFunc, WrapFunc.
// Changed: GetBindingArgumentList uses List<T> instead of BareBonesList<T>;
// InvokeLongCallSite added for invocations with more than 14 arguments.
...
namespace FSharp.Interop.Dynamic.BridgeSupport.Internal
{
    internal static partial class InvokeHelper
    {
        ...
        internal static IEnumerable<CSharpArgumentInfo> GetBindingArgumentList(object[] args, string[] argNames, bool staticContext)
        {
            ...
            var tList = new List<CSharpArgumentInfo>(args.Length + 1)
                        {
   ...
   where T : class
        ...
        // Not upstream. Dynamitey routed invocations with more than 14
        // arguments through ImpromptuInterface's Reflection.Emit to build a
        // delegate type of the right arity, and threw TypeLoadException when
        // that package was not loaded. Expression.GetDelegateType does the
        // same job from the BCL: it returns Func<>/Action<> up to 16
        // parameters and synthesizes a delegate type past that. The call site
        // is cached by the non-generic CreateCallSite like every other arity.
        internal static object InvokeLongCallSite(
            ...
            object[] args)
        {
            if (callsite == null)
            {
                var tSignature = new List<Type>(args.Length + 3) { typeof(CallSite), targetType };
                tSignature.AddRange(Enumerable.Repeat(typeof(object), args.Length));
                tSignature.Add(returnType);
                var tDelegateType = Expression.GetDelegateType(tSignature.ToArray());
                callsite = CreateCallSite(tDelegateType, binderType, knownType, binder, name, context, argNames, staticContext);
            }

            var tTarget = (Delegate)callsite.GetType().GetField("Target").GetValue(callsite);
            var tParameters = new object[args.Length + 2];
            tParameters[0] = callsite;
            tParameters[1] = target;
            Array.Copy(args, 0, tParameters, 2, args.Length);

            try
            {
                return tTarget.DynamicInvoke(tParameters);
            }
            catch (TargetInvocationException ex) when (ex.InnerException != null)
            {
                // Surface the binder's own exception (RuntimeBinderException
                // and friends) exactly as the fixed-arity paths do.
