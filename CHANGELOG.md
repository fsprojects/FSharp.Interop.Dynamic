...
- The `Dynamitey` package is no longer a dependency. The DLR call-site code the operators use is vendored from Dynamitey 3.0.3 (Apache-2.0) into a new package, `FSharp.Interop.Dynamic.BridgeSupport`, which this package depends on. `Dyn.namedArg`, `Dyn.staticTarget` and `Dyn.staticContext` now return `FSharp.Interop.Dynamic.BridgeSupport.InvokeArg` / `InvokeContext`; code that passed them to Dynamitey APIs such as `Build<_>.NewObject` or `Dynamic.Curry` must construct Dynamitey's own `InvokeArg` / `StaticContext` instead. Referencing Dynamitey alongside still works.

### Added

- `Dyn.tryGet` and `Dyn.exists`: look up a member as `'T option`, or test for it, without throwing (#42, #43).
- Dynamic invocations with more than 14 arguments work. Dynamitey routed those through ImpromptuInterface and threw `TypeLoadException` when it was not installed, which it never was here; BridgeSupport builds the delegate type with `Expression.GetDelegateType`.
